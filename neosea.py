...
def add_packet_to_neo4j(tx, packet_info):
    query = """
    ...
    SET p.packet_id = $packet_id, 
        ...
        p.label = $label
    """
    tx.run(query, packet_id=packet_info["packet_id"], protocol=packet_info["protocol"], tcp_flags=packet_info["tcp_flags"], dns_domain=packet_info["dns_domain"], http_url=packet_info["http_url"], src_ip=packet_info["src_ip"], src_port=packet_info["src_port"], src_mac=packet_info["src_mac"], dst_ip=packet_info["dst_ip"], dst_port=packet_info["dst_port"], dst_mac=packet_info["dst_mac"], size=packet_info["size"], timestamp=packet_info["timestamp"], label=packet_info["label"])
...
def process_packet(packet):
    ...
    if packet_info['dst_ip'] == chum_addr:
        print(f">>>> [{packet_info['packet_id']}] [PROTOCOL: {packet_info['protocol']} | {packet_info['tcp_flags']}] [DNS: {packet_info['dns_domain']}] [HTTP: {packet_info['http_url']}] [SRC: {packet_info['src_ip']} | {packet_info['src_port']} | {packet_info['src_mac']}] [DST :{packet_info['dst_ip']} | {packet_info['dst_port']} | {packet_info['dst_mac']}] [SIZE :{packet_info['size']}] [{packet_info['timestamp']}] <<<< {packet_info['label']} PACKET")
    else:
        print(f">>>> [{packet_info['packet_id']}] [PROTOCOL: {packet_info['protocol']} | {packet_info['tcp_flags']}] [DNS: {packet_info['dns_domain']}] [HTTP: {packet_info['http_url']}] [SRC: {packet_info['src_ip']} | {packet_info['src_port']} | {packet_info['src_mac']}] [DST :{packet_info['dst_ip']} | {packet_info['dst_port']} | {packet_info['dst_mac']}] [SIZE :{packet_info['size']}] [{packet_info['timestamp']}] <<<< {packet_info['label']} PACKET")
    ...
    try:
        with driver.session() as session:
            session.execute_write(add_packet_to_neo4j, packet_info)
