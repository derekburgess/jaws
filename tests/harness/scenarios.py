...
def background_packets(t0=0.0, seed=7):
    ...
    for ip, port, n, mtu in peers:
        ...
        for _ in range(n):
            ...
            if t > t0 + WINDOW:
                break
            rows.append(
                _pkt(HOST, ip, 44000 + rng.integers(0, 900), port, rng.integers(60, 400), t)
            )
            rows.append(
                _pkt(
                    ip,
                    HOST,
                    port,
                    44000 + rng.integers(0, 900),
                    rng.integers(200, mtu),
                    t + abs(rng.normal(0.04, 0.02)),
                )
            )
    ...
    for _ in range(60):
        ...
        rows.append(
            _pkt(
                HOST, "192.168.1.1", 5300 + rng.integers(0, 400), 53, rng.integers(60, 90), t, "UDP"
            )
        )
        rows.append(
            _pkt(
...
def slow_exfil(t0=0.0, scale=1.0, seed=14):
    ...
    while t < t0 + WINDOW:
        rows.append(
            _pkt(HOST, "203.0.113.44", 52001, 443, int(1400 * scale) + rng.integers(-80, 80), t)
        )
        if rng.random() < 0.06:
...
def burst_exfil(t0=0.0, scale=1.0, seed=15):
    ...
    for _ in range(int(500 * scale)):
        rows.append(_pkt(HOST, "203.0.113.90", 52777, 8443, 1400 + rng.integers(-60, 60), t))
...
def bulk_download(t0=0.0, scale=1.0, seed=16):
    ...
    for _ in range(int(600 * scale)):
        rows.append(_pkt("150.171.109.74", HOST, 443, 41000, 1400 + rng.integers(-60, 60), t))
        if rng.random() < 0.08:
...
def stable_heavy(t0=0.0, scale=1.0, seed=17):
    ...
    while t < t0 + WINDOW:
        rows.append(
            _pkt(HOST, "160.79.104.10", 53100, 443, int(1200 * scale) + rng.integers(-200, 200), t)
        )
        rows.append(
            _pkt(
                "160.79.104.10",
                ...
                443,
                53100,
                int(300 * scale) + rng.integers(-80, 80),
                t + 0.09,
            )
        )
